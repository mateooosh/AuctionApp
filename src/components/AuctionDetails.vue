<template>
  <div class="details">
    ...
    <div class="details__container">
      ...
      <div class="details__auction">
        <div class="details__bid">
          ...
          <div v-if="details.auctionState===1 || details.auctionState===3" class="details__left">Aukcja została
            zakończona
          </div>
          <form class="details__form__actualPrice">
            <label class="details__form__actualPrice__label" for="actualPrice"></label>
            <input v-model.number="offer" class="details__form__actualPrice__input" :min="details.maxBidPrice+1"
                   :max="details.buyNowPrice-1" name="actualPrice" id="actualPrice" type="number"
                   placeholder="Twoja oferta">
          </form>
          <button @click="bid" class="details__btn"
                  :class="{'outdated': details.auctionState == 1 || details.auctionState == 3}">
        ...
        <div class="details__buyNow">
          ...
          <button @click="buyNow" class="details__btn"
                  :class="{'outdated': details.auctionState == 1 || details.auctionState == 3}">
    ...
    <div class="details__map" v-if="mapIsVisible">
      ...
      <div class="details__location"><i class="fas fa-map-marker-alt"></i> {{ details.location }}
        ({{ details.province }})
      </div>
      <iframe
          id="map"
    ...
    fetch(url, {
      headers: {
        ...
        .then(response => {
          ...
          this.offer = parseInt(response.maxBidPrice + 1);
              ...
              headers: {
                ...
                .then((response) => {
                  ...
                  if (response.price != this.details.maxBidPrice) {
                    ...
                    this.offer = parseInt(this.details.maxBidPrice + 1);
                    // animate price
                    this.animate = true;
                    setTimeout(() => {
                      this.animate = false;
        ...
        .then(response => {
          ...
          let width = Math.floor(document.getElementsByClassName("details__map")[0].getBoundingClientRect().width - 40);
          console.log(width)
        ...
        if (this.$store.state.logged) {
          if (this.details.authorEmail === this.$store.state.userEmail) {
            ...
            if (this.offer >= this.details.buyNowPrice || this.offer <= this.details.maxBidPrice) {
              ...
              this.offer = parseInt(this.details.maxBidPrice);
            } else {
